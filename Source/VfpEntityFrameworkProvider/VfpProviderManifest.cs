...
namespace VfpEntityFrameworkProvider {
    internal class VfpProviderManifest : DbXmlEnabledProviderManifest {
        ...
        private static XmlReader GetStoreSchemaMapping(string informationType) {
            if (informationType == StoreSchemaMappingVersion3) {
                return XmlReader.Create(new StringReader(Properties.Resources.StoreSchemaMappingVersion3));
            }

            return XmlReader.Create(new StringReader(Properties.Resources.StoreSchemaMapping));
        }

        private static XmlReader GetStoreSchemaDescription(string informationType) {
            if (informationType == StoreSchemaDefinitionVersion3) {
                return XmlReader.Create(new StringReader(Properties.Resources.StoreSchemaDefinitionVersion3));
            }
